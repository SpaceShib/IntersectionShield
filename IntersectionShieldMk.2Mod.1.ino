...
void loop() {
  if (triggered) {
    if((currentState == 1) || (currentState == 4)){   //If state is on a GREEN
      if(countdown == 0){   //If countdown is at the end
        currentState++;   //transition to next state
        countdown = 7;    //reset countdown
      }//End if
      else{   //While counting down
